...
export default async function AnalyticsPage() {
  ...
  const storageWarnings = summaries.filter((summary) => summary.message || summary.storageConfigured === false);
      ...
      <div className="mx-auto max-w-7xl">
        <header className="border-b border-[#D8D1C6] pb-8">
          ...
          {storageWarnings.length > 0 ? (
            <div className="mt-6 border-l-4 border-[#C33124] bg-[#FFF1EE] p-5">
              <p className="text-lg font-black text-[#7A271A]">保存先の設定が必要です。</p>
              <p className="mt-2 leading-8 text-[#7A271A]">
                {storageWarnings.map((item) => item.siteLabel).join("、")} の数字は、永続保存先が未設定です。
                Vercel Blobを接続すると、本番公開後も数字が残ります。
              </p>
            </div>
          ) : null}
        ...
        <section className="border-t border-[#D8D1C6] py-10">
          <div className="bg-[#102A43] p-6 text-white sm:p-8">
            ...
            <p className="mt-4 max-w-4xl leading-8 text-white/80">
              この数字は、専用集計を公開した後からたまります。Vercel Blobに保存されるため、
              公開後もページ表示、ボタン、フォーム送信の数字を確認できます。
            </p>
